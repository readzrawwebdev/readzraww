...
const OrderForm = ({ plan, onClose }: Props) => {
  ...
  const handleUpload = () => {
    ...
    setUploading(true);
    // Simulate upload — will be replaced with Cloud storage
    setTimeout(() => {
      setUploading(false);
      setStep("success");
    }, 2000);
        ...
        >
              ...
              {(["form", "payment", "upload"] as Step[]).map((s, i) => (
                <div
                  ...
                  className={`h-1.5 flex-1 rounded-full ${
                    (["form", "payment", "upload"] as Step[]).indexOf(step) >= i
                      ? "bg-gradient-primary"
                      : "bg-secondary"
                  }`}
                />
          ...
          {step === "form" && (
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full rounded-lg bg-gradient-primary py-3 font-semibold text-primary-foreground transition-transform hover:scale-[1.02]"
              >
                Continue to Payment
              </button>
            </form>
          ...
          {step === "payment" && (
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full rounded-lg border border-border py-3 font-semibold text-secondary-foreground transition-colors hover:bg-surface-hover"
              >
                Go Back
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div
                className={`rounded-xl border-2 border-dashed p-8 text-center transition-colors ${
                  receiptFile ? "border-accent bg-accent/5" : "border-border"
                }`}
              >
                {receiptFile ? (
          ...
          {step === "success" && (
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground text-sm">
                Your website will be ready in <span className="font-bold text-foreground">5 days</span>.
                We'll review your receipt and get started right away.
              </p>
              <div className="rounded-xl bg-secondary p-4 text-sm text-muted-foreground">
                For more info, contact us at{" "}
                <a href="mailto:readzraw@gmail.com" className="text-primary font-medium">
                  readzraw@gmail.com
                </a>
              </div>
              <button
